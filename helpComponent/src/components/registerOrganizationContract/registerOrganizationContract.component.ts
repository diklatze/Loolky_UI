...
@Component({
    ...
    templateUrl: 'registerOrganizationContract.component.html',
    providers: [RegisterContractServices]
})
export class RegisterOrganizationContractPage implements OnInit {
    pageTitle: string;

    messages = null;
    organizarionName: string;
    ...
    constructor(private formBuilder: FormBuilder, private registerContractServices: RegisterContractServices, private navController: NavController, private memberServices: MemberServices, private _MessageServices: MessageServices, private pageService: PageService) {
        this.organizarionName = this.pageService.getMember().fullName;
        this.organizationShortName = this.pageService.getMember().shortName;
        this.member = this.pageService.getMember();
        this.pageTitle = this.organizarionName + " - Register Contract";

        this.memberServices.getMemberDetailsAndContractValidation(this.organizationShortName, "Organization").subscribe(
            res => {
                console.log(res);
                ...
                switch (res.code) {
                    case 0:
                        ...
                        break;
                    case 586:
                        this._MessageServices.getMessage(Constants.MESSAGE_MUST_CREATE_ACCT, (resultMessage: string) => { this.messageContent = resultMessage }, undefined, [this.organizarionName]);
    ...
    readMemberAccountAndSetErrorMessage(response: any): void {
        ...
        if (response.body.contract != null) {
            ...
            this._MessageServices.getMessage(Constants.MESSAGE_MUST_CREATE_ACCT, (resultMessage: string) => { this.messageContent = resultMessage; console.log("Error message " + this.messageContent); }, undefined, [this.organizarionName]);
        }
    }
